document.addEventListener("DOMContentLoaded", function() {
  ...
  function addMarkerWithPopup(eventData) {
    ...
    // colorer les markers en fonction du delai
    // la date du jour de consultation
    let currentDate = Date.now();

    // récupérer la date de l'event
    let eventDate = new Date(eventData.start).getTime();
    ...
    let color;
    if(daysDiff > 3) {
      // pour un event dans plus de trois jours
      color = 'green';
    } else if (daysDiff <= 3 && daysDiff >= 0) {
      // pour un event dans trois jours ou moins
      color = 'orange';
    } else {
      // event passé
      color = 'red';
